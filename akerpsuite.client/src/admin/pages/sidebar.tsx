...
export default function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
    ...
    // 🆕 Logged-in employee's own photo (for the bottom profile avatar)
    ...
    // 🆕 Fetch the logged-in user's own profile photo — independent of everything else,
    // fails silently since the initials avatar is already a fine fallback.
    useEffect(() => {
        adminService
            .getMyProfile()
            .then((res) => {
            ...
            <aside className="h-full w-full flex flex-col bg-[#0b2836] text-white font-sans">
                        ...
                        if (items.length === 1) {
                            ...
                            return (
                                <Link
                                    ...
                                    to={page.pageLink}
                                    onClick={onNavigate}
                                    className={`relative flex items-center gap-3 py-2.5 pl-4 pr-3 rounded-xl transition-all ${active
                        ...
                        return (
                            <div key={groupName} className="select-none">
                                <button
                                    ...
                                    className={`relative w-full flex items-center justify-between py-2.5 pl-4 pr-3 rounded-xl transition-all ${hasActiveChild && !isOpen
                                ...
                                >
                                    ...
                                    <i
                                        className={`fa-solid fa-chevron-down text-[10px] text-slate-500 transition-transform duration-300 ${isOpen ? "rotate-180" : "rotate-0"
                                            }`}
                                    />
                                </button>
                                            ...
                                            {items.map((page) => {
                                                ...
                                                return (
                                                    <Link
                                                        ...
                                                        to={page.pageLink}
                                                        onClick={onNavigate}
                                                        className={`relative py-1.5 pl-4 pr-2 rounded-lg text-[12.5px] transition-all ${active
                ...
                <div className="px-4 pt-3 pb-4 border-t border-white/10">
                    <div className="flex items-center gap-2.5">
                        {/* 🆕 Photo avatar if available, initials fallback otherwise */}
                        <div className="w-9 h-9 rounded-full bg-[#cfcb9e] flex items-center justify-center shrink-0 overflow-hidden">
                            {myPhotoPath ? (
                                <img
                                    ...
                                    onError={(e) => {
                                        // If the photo fails to load, fall back to initials by clearing the path
